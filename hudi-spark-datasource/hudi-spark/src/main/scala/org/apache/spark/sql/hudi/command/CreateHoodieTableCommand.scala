...
import org.apache.spark.sql.hudi.{HoodieSqlUtils, SQLConfig}
...
case class CreateHoodieTableCommand(table: CatalogTable, ignoreIfExists: Boolean)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    var path = HoodieSqlUtils.getTableLocation(table, sparkSession)
    ...
    val tableType = SQLConfig.getTableType(table.storage.properties)
    val inputFormat = tableType match {
      case DataSourceWriteOptions.COW_TABLE_TYPE_OPT_VAL =>
        classOf[HoodieInputFormat].getCanonicalName
...
object CreateHoodieTableCommand extends Logging {
  ...
  def initTableIfNeed(sparkSession: SparkSession, table: CatalogTable): Unit = {
    ...
    if (!tableExists) {
      ...
      HoodieTableMetaClient.initTableType(conf, basePath.toString,
        HoodieTableType.valueOf(tableType), tableName, payloadClass)
    }
  }
}
