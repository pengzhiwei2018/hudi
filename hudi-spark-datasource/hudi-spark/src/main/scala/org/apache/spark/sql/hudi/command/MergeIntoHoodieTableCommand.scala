...
import org.apache.hudi.config.HoodieIndexConfig.BLOOM_INDEX_PRUNE_BY_RANGES_PROP
...
import org.apache.hudi.config.{HoodieMetricsConfig, HoodieWriteConfig}
...
import org.apache.spark.sql.hudi.{SQLConfig, SerDeUtils}
import org.apache.spark.sql.hudi.command.payload.ExpressionPayload
import org.apache.spark.sql.hudi.logical.{Assignment, DeleteAction, InsertAction, MergeIntoTable, UpdateAction}

/**
  * The Command for hoodie MergeIntoTable. It uses the Hoodie index for matching the source and the
  * target.The data matched the merge-on condition will go to ExpressionPayload#combineAndGetUpdateValue
  * while the data not matched goes to the ExpressionPayload#getInsertValue. And the action
  * expressions will pushed down and be executed in the ExpressionPayload's related methods.
  *
  * @param mergeInto
  */
case class MergeIntoHoodieTableCommand(mergeInto: MergeIntoTable) extends RunnableCommand {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val parameters = configHoodieWriteParameters(mergeInto)
  ...
  /**
    * Execute the upsert operator. All the matched and not-matched actions will execute in
    * one upsert write operation. We pushed down the matched condition and assignment expressions
    * to the ExpressionPayload#combineAndGetUpdateValue and the not matched expressions to the
  ...
  /**
    * Create the basic config for hoodie writer.
    * @param mergeInto
    * @return
    */
  private def configHoodieWriteParameters(mergeInto: MergeIntoTable): Map[String, String] = {
    ...
    val definedPk = SQLConfig.getPrimaryColumns(options)
