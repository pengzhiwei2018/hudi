...
import org.apache.spark.sql.catalyst.expressions.{Alias, And, AttributeReference, Cast, Expression, Literal}
...
object HoodieSqlUtils {
  ...
  /**
    ...
    * We add the "spark" prefix to  hoodie's config key when we pass
    * the config to spark conf. So here we need to remove the prefix.
    * @param spark
    * @param options
    * @param baseConfig
    * @return
  ...
  def withSparkConf(spark: SparkSession, options: Map[String, String])
    ...
    baseConfig ++ // Table options has the highest priority
      (spark.sessionState.conf.getAllConfs ++ SQLConfig.mappingSqlOptionToHoodieParam(options))
        .filterKeys(_.startsWith(SQLConfig.CONFIG_PREFIX)).map {
        case (key, value) => (SQLConfig.tripPrefix(key), value)
      }
  }

  def containUnResolvedStar(project: Project): Boolean = {
    project.projectList.exists(_.isInstanceOf[UnresolvedStar])
  }

  def containMetaField(project: Project): Boolean = {
  ...
  def removeMetaField(project: Project): Project = {
