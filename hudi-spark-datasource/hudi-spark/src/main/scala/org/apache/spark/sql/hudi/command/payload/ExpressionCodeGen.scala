...
object ExpressionCodeGen extends Logging {
  ...
  def doCodeGen(exprs: Seq[Expression]): IExpressionEvaluator = {
    ...
    val replacedExprs = exprs.map(replaceBoundReference(_))
    val resultVars = replacedExprs.map(_.genCode(ctx))
    val className = s"ExpressionPayloadEvaluator_${UUID.randomUUID().toString}"
    val codeBody =
      s"""
         |
    ...
    evaluator.getClazz.newInstance().asInstanceOf[IExpressionEvaluator]
  }

  /**
    * Replace the BoundReference to RecordBoundReference which will override the implement
    * of CodeGen.
    * @param expression
    * @return
    */
...
case class RecordBoundReference(ordinal: Int, dataType: DataType, nullable: Boolean)
  ...
  /**
    ...
    * This implementation is referred to BoundReference#doGenCode. The mainly different between
    * BoundReference#doGenCode is that RecordBoundReference's input object is an IndexedRecord
    * while BoundReference's is a Row. The code for getting field from input object is different.
    * @param ctx
    * @param ev
    * @return
    */
